...
module.exports = {
  ...
  loadPage: function(opts, bodyCb, cb){
    ...
    .onkeep(function(got){
      try {
        ...
        if (!opts.replace){
          /*
           * This is a full page load. Updates the title
           * and resets focus and scroll position.
           */
          bodyCb(oldBody, newBody);
          ...
          /*
           * This is kind of a quasi page load. It updates
           * document title, but doesn't disturb the state of
           * the page, rather updates a sub-region of the page.
           */
          var $goingAway = $(oldDoc).find(opts.replace).eq(0);
          var $comingIn = $(newDoc).find(opts.replace).eq(0);
          $goingAway.replaceWith($comingIn);
        }
      } catch(ex) {
        cb(ex);
        return;
